    ...
    <section id="solutions" className="bg-white py-16 lg:py-24">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-on-scroll">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <p className="text-[#1D4ED8] text-xs font-bold uppercase tracking-[0.3em]">SOLUTIONS</p>
        </div>
        <h2 className="text-3xl lg:text-4xl font-extrabold text-[#0F172A] mb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {solutions.map((sol, i) => (
            <div
              key={i}
              className="relative bg-[#EFF6FF] rounded-lg p-8 overflow-hidden group hover:shadow-lg transition-shadow"
            >
              {/* Watermark number */}
              <span className="absolute -top-3 -right-1 text-8xl font-extrabold text-[#0F172A] opacity-5 leading-none select-none pointer-events-none">
                {sol.number}
              </span>

              {/* Icon box */}
              <div className="w-12 h-12 border-2 border-[#1D4ED8] rounded-lg flex items-center justify-center mb-5 group-hover:bg-[#1D4ED8] transition-colors">
              ...
              {/* Number label */}
              <p className="text-[#1D4ED8] text-xs font-bold mb-2 tracking-widest">{sol.number}</p>

              <h3 className="text-xl font-extrabold text-[#0F172A] mb-3 leading-snug">
                {sol.title}
              </h3>
              ...
              {/* Bottom accent line */}
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#1D4ED8] opacity-0 group-hover:opacity-100 transition-opacity rounded-b-lg" />
            </div>
          ))}
        </div>
